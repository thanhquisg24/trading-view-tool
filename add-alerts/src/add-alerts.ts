    ...
    try {
      ...
      readStream
        ...
        .pipe(
          csv.parse({
            headers: (headerArray) =>
              headerArray.map((header) => header.trim()),
          })
        )
...
export const addAlertsMain = async (configFileName) => {
  ...
  if (
    ...
    log.fatal(
      "oops! Looks like you need to set your chartUrl in the config file!"
    );
    process.exit(1);
  }

  let blackListRows = [];
  log.info("----blackListRows-----", blackListRows);
  log.trace(
    `${kleur.gray("Reading input file: ")}${kleur.cyan(config.files.input)}`
  );
  ...
  if (!firstRow.symbol) {
    log.error(
      `Invalid input csv file format, first line should have at least the following headers(no spaces!): ${kleur.blue(
  ...
  if (alertConfig.actions.playSound && alertConfig.actions.playSound.enabled) {
    if (!isSoundName(alertConfig.actions.playSound.name)) {
      log.error(
        `Configuration error. Invalid sound name: ${kleur.yellow(
          alertConfig.actions.playSound.name
        )}`
      );
    ...
    if (!isSoundDuration(alertConfig.actions.playSound.duration)) {
      log.error(
        `Configuration error. Invalid sound duration: ${kleur.yellow(
          alertConfig.actions.playSound.duration
        )}`
      );
      throw new Error(
        `Sound Duration must be one of: ${soundDurations.join(" , ")}`
      );
    }
  }
  ...
  const browser: Browser = await launchBrowser(
    headless,
    `${config.tradingview.chartUrl}#signin`
  );
  ...
  for (const row of symbolRows) {
    try {
        ...
        if (value) {
          ...
          if (matches) {
            for (const match of matches) {
              log.warn(
                `No key in .csv matches '${match}' - but might be using TradingView token-replacement`
              );
            }
          }
      ...
      const parsedIntervals = replacedIntervals.toString().split(",") || [
        "none",
      ];

      for (const currentInterval of parsedIntervals) {
        log.info(
          `Adding symbol: ${kleur.magenta(
            row.symbol
          )} | Instrument: ${kleur.magenta(
            row.instrument || row.base
          )} Quote Asset: ${kleur.magenta(row.quote_asset || row.quote)}`
        );
        ...
        const singleAlertSettings: ISingleAlertSettings = {
          name: makeReplacements(
            row.alert_name || row.name || alertConfig.name
          ), // TODO: deprecate "name" one day
          ...
          condition: {
            ...
            tertiaryRight: makeReplacements(
              alertConfig.condition.tertiaryRight
            ),
            quaternaryLeft: makeReplacements(
              alertConfig.condition.quaternaryLeft
            ),
            quaternaryRight: makeReplacements(
              alertConfig.condition.quaternaryRight
            ),
          },
