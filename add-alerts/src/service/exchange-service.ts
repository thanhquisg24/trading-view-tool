...
const fetchAndTransform = async (
  url,
  responsePath,
  transformer: (any) => MasterSymbol
) => {
  const responseObject = await fetch(url);

  const resultsArray = responsePath
    ? responseObject.data[responsePath]
    : responseObject.data;
    ...
    if (obj.suspended === false) {
      ...
      if (obj.tag === "perpetual") {
        ...
        return new MasterSymbol(
          ...
          obj.symbol,
          quoteCurrency,
          `KRAKEN:${baseCurrency}${quoteCurrency}PERP`,
          classification
        );
      } else {
        // TODO: dated futures
...
export const fetchBittrex = async (): Promise<MasterSymbol[]> => {
  const transformer = (obj) => {
    if (obj.status === "ONLINE") {
      const classification: ClassificationType = obj.baseCurrencySymbol.match(
        /X\s(?:BULL|BEAR)\s/
      )
        ? Classification.LEVERAGED_TOKEN
        : Classification.SPOT;
      return new MasterSymbol(
        obj,
        BITTREX,
        obj.quoteCurrencySymbol,
        obj.baseCurrencySymbol,
        null,
        classification
      );
    } else {
...
export const fetchFtx = async (): Promise<MasterSymbol[]> => {
  const levTokensTransformer = (obj) => {
    return new MasterSymbol(
      ...
      "USD",
      `FTX:${obj.underlying}`,
      Classification.FUTURES_DATED
    );
  };
  const levTokenSymbols = await fetchAndTransform(
    "https://ftx.com/api/lt/tokens",
    "result",
    levTokensTransformer
  );
  ...
  const transformer = (obj) => {
    if (obj.enabled) {
      if (obj.type === "spot") {
        const classification: ClassificationType = levTokenNames.includes(
          obj.baseCurrency
        )
          ? Classification.LEVERAGED_TOKEN
          : Classification.SPOT;
        ...
        if (obj.name.match(/-PERP$/)) {
          return new MasterSymbol(
            ...
            obj.name,
            "USD",
            `FTX:${obj.underlying}PERP`,
            Classification.FUTURES_PERPETUAL
          );
        } else if (obj.name.match(/-\d{4}$/)) {
          //AVAX-0326
  ...
  const transformer = (obj) => {
    if (obj.ctType === "inverse") {
      ...
      return new MasterSymbol(
        ...
        obj.instId,
        obj.ctValCcy,
        `OKEX:${obj.ctValCcy}${obj.settleCcy}PERP`,
        Classification.FUTURES_PERPETUAL
      );
    } else {
...
export const fetchSymbolsForSource = async (
  source: string
): Promise<MasterSymbol[]> => {
  ...
  switch (source) {
    ...
    default:
      log.error(
        `Invalid source specified: ${kleur.yellow(
          source
