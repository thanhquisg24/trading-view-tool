...
export const logBaseDelay = () => {
  log.info(
    `Base delay: ${kleur.yellow(
      BASE_DELAY
    )} (you can specify '--delay 1000' (or env variable BASE_DELAY) to increase/decrease speed)`
  );
};
...
export const waitForTimeout = (
  ...
  if (millsOrMultplier < 20) {
    ...
    log.trace(
      kleur.gray(`...waiting ${kleur.bold().white(waitTime)}ms   ${message}`)
    );
  }
...
export const isEnvEnabled = (envVariable: string) => {
  return (
    !!envVariable && (envVariable === "true" || Boolean(Number(envVariable)))
  );
};
